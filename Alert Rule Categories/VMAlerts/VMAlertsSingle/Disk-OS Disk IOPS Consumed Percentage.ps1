...
# Define variables
$alertRuleNamePrefix = "VM - Disk - OS Disk IOPS Consumed Percentage - azm "
...
foreach ($vm in $vms) {
    try {
        ...
        Add-AzMetricAlertRuleV2 -Name $alertRuleName `
            ...
            -Severity 4 `
            -Description "Client Name:$Customer - Input/Output Operations Per Second for virtual machine OS (VM) disks on VM $($vm.Name)"
