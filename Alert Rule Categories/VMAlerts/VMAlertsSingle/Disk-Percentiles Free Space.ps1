...
# Define variables
$alertRuleNamePrefix = "VM - Disk - Percentiles Free Space - azm "
...
foreach ($vm in $vms) {
    try {
        ...
        $description = "Client Name:$Customer - Measure the percentage of free disk space on all logical disks (partitions) on an OS system on VM $vmName"
