...
# Define variables
$alertRuleNamePrefix = "VM - Memory - Virtual Machines by AvailableMB - azm "
...
$condition = New-AzScheduledQueryRuleConditionObject `
    -Dimension $dimension `
    -Query "Perf | where TimeGenerated > ago(30min) | where ObjectName in ('LogicalDisk', 'Logical Disk') | where (InstanceName == 'C:' or InstanceName == '/') | where CounterName == 'Free Megabytes' | summarize MaxAvailableMBytes = max(CounterValue) by bin(TimeGenerated, 5m), Computer, _ResourceId, InstanceName" `
